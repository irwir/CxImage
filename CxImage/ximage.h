...
typedef struct tagCxTextInfo
{
    ...
    int32_t     align;      ///< DT_CENTER, DT_RIGHT, DT_LEFT alignment for multiline text
...
/** \addtogroup Interpolation */ //@{
	...
	RGBQUAD GetPixelColorWithOverflow(int32_t x, int32_t y, OverflowMethod const ofMethod=OM_BACKGROUND, const RGBQUAD* const rplColor=0);
	...
	RGBQUAD GetPixelColorInterpolated(float x,float y, InterpolationMethod const inMethod=IM_BILINEAR, OverflowMethod const ofMethod=OM_BACKGROUND, const RGBQUAD* const rplColor=0);
...
protected:
/** \addtogroup Protected */ //@{
	bool EncodeSafeCheck(const CxFile *hFile);
//@}
...
/** \addtogroup Transformations */ //@{
	...
	bool Rotate2(float angle, CxImage *iDst = NULL, InterpolationMethod inMethod=IM_BILINEAR,
                OverflowMethod ofMethod=OM_BACKGROUND, const RGBQUAD* const replColor=0,
...
/** \addtogroup DSP */ //@{
	...
	float Mean();
	bool Filter(const int32_t* const kernel, int32_t Ksize, int32_t Kfactor, int32_t Koffset);
	...
	bool Lut(const uint8_t* const pLut);
	bool Lut(const uint8_t* const pLutR, const uint8_t* const pLutG, const uint8_t* const pLutB, const uint8_t* const pLutA = 0);
...
/** \addtogroup Protected */ //@{
	...
	static void blur_line (float *ctable, const float* const cmatrix, int32_t cmatrix_length, uint8_t* cur_col, uint8_t* dest_col, int32_t y, int32_t bytes);
