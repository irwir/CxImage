...
void CxImage::HuePalette(float correction)
{
	...
	correction *= 255.0f / (head.biClrUsed - 1);
	for(uint32_t j=0; j<head.biClrUsed; j++){
		uint8_t i=(uint8_t)(j*correction);
		RGBQUAD hsl={120,240,i,0};
...
bool CxImage::Jitter(int32_t radius)
{
	...
	for(int32_t y=ymin; !info.nEscape && y<ymax; y++){
		info.nProgress = (int32_t)(100*(y-ymin)/(ymax-ymin));
		for(int32_t x=xmin; x<xmax; x++){
#if CXIMAGE_SUPPORT_SELECTION
			if (BlindSelectionIsInside(x,y))
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				int nx=x+(int32_t)((rand()/(2 * RAND_MAX - 1) * radius));
				int ny=y+(int32_t)((rand()/(2 * RAND_MAX - 1) * radius));
				if (!IsInside(nx,ny)) {
					nx=x;
					ny=y;
				}
...
int32_t CxImage::gen_convolve_matrix (float radius, float **cmatrix_p)
{
	...
	*/
	...
	for (i = matrix_length/2 + 1; i < matrix_length; i++)
    {
		...
		float base_x = (float)(i - (2 * matrix_length + 1) / 2);
		sum = 0;
		for (j = 1; j <= 50; j++)
		{
